 ...
 *   4. Per NU data generated by the pipeline stays in js/data.js. This file
 *      holds constants, plus the per NU tables the pipeline does not
 *      generate (A12, 2026-09-09: CONCEPTS, ENERGY_COLORS,
 *      ENERGY_STATUS_IMAGES, GFA_DATA, CONDITIONED_AREA_DATA, PV_AREA_DATA).
...
const LMN_CONFIG = {
  ...
  // The JSON export (js/finish-design.js, js/comparison.js) reads this
  // object rather than carrying its own copy.
  attribution: {
  ...
  // js/comparison.js are committed (A27, 2026-09-09) but linked from nothing,
  // gated by this flag alone. The final results page still carries the entry
  // point, so the button has to be told to stay away. An entry point that
...
/* ---------------------------------------------------------------------------
 * A12, 2026-09-09: six blocks moved here from js/data.js. Every one of them
 * was hand-maintained inside that file even though the file is generated;
 * `Templates/scripts/patch_data_js.py` only ever patches NEIGHBOURHOODS,
 * PV_GENERATION_DATA and ENVELOPE_ENERGY_DATA, and merges into BUILDING_IMAGES.
 * Global names are unchanged, so no call site in js/ changed. EV_V2G_DATA and
 * LPV_DATA stayed in js/data.js: they are regenerated by
 * Templates/Content_Layer3_Transportation/generate_ev_lpv_data.py, so they are
 * not hand-maintained. ENERGY_DATA_ARCHIVED also stayed: it is already wrapped
 * in a block comment, dead since before this pass, so moving it would add
 * inert text to a file whose own rule 3 is constants only.
 * ------------------------------------------------------------------------- */
...
//
// The pages that read the value only to build the next page's link kept their
// own expression until A15. On 2026-09-09 the sixteen of them, in eight files,
// dropped the literal too: with no climate chosen the link now carries an empty
// envelope and the page it opens states the refusal. No silent Montreal remains
// anywhere in js/.
  ...
  const initTerminologyLegendEvents = function () {
    ...
    try {
      ...
      document.addEventListener("keydown", function (e) {
        ...
        // DBG-053, 2026-09-09. The card information icons are spans carrying
        // role="button" and tabindex="0", so they take tab focus, but a span
        // turns neither Enter nor Space into a click the way a real <button>
        // does. Without this branch the seven of them are reachable by
        // keyboard and cannot be opened by it. The other three popovers are
        // real buttons and need nothing.
        if (e.key === "Enter" || e.key === " " || e.key === "Spacebar") {
          const keyTarget = e.target;
          const keyCardBtn = (keyTarget && typeof keyTarget.closest === "function") ? keyTarget.closest(".card-info-btn") : null;
          if (keyCardBtn) {
            if (typeof e.preventDefault === "function") { e.preventDefault(); }
            if (typeof keyCardBtn.click === "function") { keyCardBtn.click(); }
          }
        }
      });
